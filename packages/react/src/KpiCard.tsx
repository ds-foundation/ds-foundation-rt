...
function KpiCard({ label, value, trend, icon, className }: KpiCardProps) {
  ...
  return (
    <div className={cn("border border-border rounded-xl shadow-sm bg-card p-6 flex flex-col gap-3", className)}>
      <div className="flex items-center justify-between">
        <Caption className="text-muted-foreground uppercase tracking-wide font-medium">{label}</Caption>
        {icon && <span className="text-muted-foreground">{icon}</span>}
      </div>
      <H2 className="text-foreground">{typeof value === "number" ? value.toLocaleString() : value}</H2>
      {trend && (() => {
